...
router.get('/customer', async function (req, res) {
  ...
  var poAsset = {};
  try {
    var items = await bcUtils.queryBlockchain("queryPOByBuyer", [ req.user.username ]);
    for( i in items ) {
...
router.get('/poBuyer', async function (req, res) {
  ...
  var poAsset = {};
  try {
    var items = await bcUtils.queryBlockchain("queryPOByBuyer", [ req.user.username, "open" ]);
    for( i in items ) {
      console.log( "GET::item="+JSON.stringify(items[i]));
      poAsset = poUtils.cleanPO(items[i]);
...
router.get('/poSupplier', async function (req, res) {
  ...
  var poAsset = {};
  try {
    var items = await bcUtils.queryBlockchain("queryPOBySupplier", [ req.user.username, "open" ]);
    for( i in items ) {
      console.log( "GET::item="+JSON.stringify(items[i]));
      poAsset = poUtils.cleanPO(items[i]);
      //poAsset.PODate = bcUtils.getNOW();
...
router.get('/poDetail/:id', async function (req, res) {
  ...
  var poAsset = {};
  try {
    var item = await bcUtils.queryBlockchain("queryPO", [ req.params.id ]);
